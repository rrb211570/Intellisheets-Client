...
class SheetPanel extends React.Component {
            ...
            <div>
                ...
                <div id="pageID" className="page">
                    <Provider store={store}>
                        <SpreadSheetContainer rows={DEFAULTROWS} cols={DEFAULTCOLS} rowHeight={DEFAULTROWHEIGHT} colWidth={DEFAULTCOLWIDTH} whichTests={[]} />
                    </Provider>
                </div>
            </div>
        );
    ...
    componentDidMount() {
        let id = window.location.href.match(/.+sheet\/(.+)/)[1];
        this.loadSheetAPI(id)
            .then(res => {
                if (res.status == 'success') {
                    this.setState({ sheetID: id, autoSaveTimer: setInterval(() => {
